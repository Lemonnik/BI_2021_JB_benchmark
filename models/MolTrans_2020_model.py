...
class MolTransModel(DtiModel, nn.Sequential):
    ...
    References
    ----------
    * Huang K., Xiao C., Glass LM., Sun J.
      MolTrans: Molecular Interaction Transformer for drug–target interaction prediction.
      Bioinformatics. 2021; 37(6): 830-836.
    ...
    def __init__(self, **config):
        ...
        self.decoder = nn.Sequential(
            ...
            # output layer
            nn.Linear(32, 1)  # CHANGED (32, 1) --> (32, 2) in order to get (b_size, 2)
        )
    ...
    def forward(self, d, p, d_mask, p_mask):
        ...
        i_v = i.view(int(self.batch_size / max(self.gpus, 1)), -1, self.max_d, self.max_p)
        ...
        f = f.view(int(self.batch_size / max(self.gpus, 1)), -1)
        # print(f.shape)
    ...
    def __call__(self, data, train=True, device='cpu'):
        ...
        # TODO: Move to DEVICE
        # print(d, p, d_mask, correct_interaction)
        # print([i.shape for i in data])
        # [torch.Size([16, 50]), torch.Size([16, 545]), torch.Size([16, 50]), torch.Size([16, 545]), torch.Size([16])]

        correct_interaction = correct_interaction.type(torch.LongTensor).to(device)
        d, p, d_mask, p_mask = [i.type(torch.LongTensor).to(device) for i in [d, p, d_mask, p_mask]]
        scores = self.forward(d, p, d_mask, p_mask)

        if train:
            # loss_fnc = torch.nn.BCELoss()
            # print(scores)
            # print(scores.shape)
            # print(torch.nn.Sigmoid(scores.detach()), torch.nn.Sigmoid(scores.detach()).shape)
            # print(torch.squeeze(torch.nn.Sigmoid(scores)), torch.squeeze(torch.nn.Sigmoid(scores)).shape)
            # loss = F.mse_loss(torch.squeeze(torch.nn.Sigmoid(scores)),
            #                   correct_interaction.to(torch.float32))
            loss = F.mse_loss(torch.squeeze(scores), correct_interaction.to(torch.float32))
            return loss
        else:
